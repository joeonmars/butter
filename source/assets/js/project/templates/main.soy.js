...
btr.templates.Main.ProjectCreator = function(opt_data, opt_ignored) {
  return '<div class="dialog"><label>Create a new document</label><fieldset>title: <input type="text" value="' + opt_data.data['title'] + '">width: <input type="number" step="1" value="' + opt_data.data['width']['value'] + '" min="' + opt_data.data['width']['min'] + '" max="' + opt_data.data['width']['max'] + '">height: <input type="number" step="1" value="' + opt_data.data['height']['value'] + '" min="' + opt_data.data['height']['min'] + '" max="' + opt_data.data['height']['max'] + '"></fieldset><fieldset><button class="ok">OK</button><button class="cancel">Cancel</button></fieldset></div>';
};
